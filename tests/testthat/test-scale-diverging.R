test_that("show diverging palettes", {
  bg_pal <-
    scales::show_col(pal_diverging(palette = "blue_green", alpha = 1)(5))
  vdiffr::expect_doppelganger("blue-green-palette", bg_pal)

  by_pal <-
    scales::show_col(pal_diverging(palette = "blue_yellow", alpha = 0.5)(5))
...
test_that("scatter plot with diverging palette", {
  point_plot <-
    ggplot2::ggplot(mtcars, ggplot2::aes(disp, wt, fill = drat)) +
